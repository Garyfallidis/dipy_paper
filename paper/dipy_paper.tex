...
generate streamlines\citep{Morris2008}. Streamlines are generated using a
...
associated with the measured diffusion signal \citep{Jeurissen2011}. The user must
...
example, residual bootstrap fiber tracking methods fall into this category \citep{Berman2008}.
...
\texttt{track_density} is a 3d volume where the intensity of each voxel is the
...
Here because we've made the matrix symmetric, `M[i, j] == M[j, i]` is the number
of streamlines in \texttt{track} that connect region \texttt{i} to \texttt{j}
or \texttt{j} to \texttt{i}. Similarly because we've set
\texttt{return_mapping=True}, we can get a list of all those streamlines by
doing \texttt{M_sl[i, j]}.
