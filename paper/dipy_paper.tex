...
% I think this may be rather too detailed and repetitive given what is in
...
% What does this 'rounded' mean? Is it good? Does it produce good visualisations?
% The normalization issue is something separate, I think.
...
\citet{yeh-etal:10}. We concentrated on creating a simple
...
% Is someone writing this and providing a code snippet example?
...
It is often useful in dMRI to find which ROIs a streamline passes through
or even to count the number streamlines that go through a particular ROI,
...
\end{python}
Given a set of streamlines and the shape of the 3D volume from which the streamlines
were created, we determine whether a point of a streamline passed through a voxel by
rounding the point values (in mm) to voxel coordinates (integers). For a streamline that passes through a
voxel more than once, we only increase the count for the first time
...
\texttt{tcs[i, j, k]} is the number of tracks that pass through the voxel with
coordinates (i, j, k). If \texttt{return\_elements} is True we also return an array 
\texttt{tes} of the same shape as the 3D volume of lists of
integer indices of the streamlines that pass through
the voxel.
...
% Not sure what 'up to the point' is supposed to mean?
