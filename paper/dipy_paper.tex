...
There are many other software packages to analyze {dMRI}. We designed dipy to
differ from most other packages in several ways.  It is written in Python, so
the code is easier to read, write and interact with than lower-level compiled
languages like C, C++ and Java.  We have designed our implementations to be
easy to understand and compare so dipy can be a general public library for
new and standard implementations. Dipy is not being developed by a single
research group; rather, it is a fully open project that encourages
contributions from any scientist/developer through the Github open web
interface and public discussions on the project mailing list.  Dipy today has
an international team of contributors, spanning seven different academic
institutions in five countries and three continents, which is still growing.
...
transform in eq.~\ref{eq:fourier} we can get back a 3D probability density
function (PDF) for every voxel.  The PDF is also known as the diffusion
propagator. In practice, unfeasibly long acquisition time is
...
\end{python}
Finally, a couple more points about the implementation of diffusion tensor
...
SH basis up to very high orders, in essence fitting more parameters than data
allow.
